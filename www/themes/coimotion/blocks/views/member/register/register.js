...
ctrl.doJoinNow = function() {
	...
	var  freeRegister = '<%=ph.register%>',
		 waiting = '<%=ph.js_WAITING%>';
	
	ctrl.sel('button.regBtn').find('strong').empty().html(waiting);
	...
	__.api( req, function(data) {
		if (data.errCode === 0) {
			...
			ctrl.focusAccNme();
			ctrl.clearPw();
			ctrl.sel('button.regBtn').find('strong').empty().html(freeRegister);
		}
		else {
			alert( data.message );
			ctrl.focusAccNme();
